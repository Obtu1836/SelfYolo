...
class VOCDection(Dataset):
    ...
    def load_image_target(self,index):
        ...
        h,w,c=image.shape
        anno=ET.parse(Path(self.anno_path.format(path=path,name=name))).getroot()#解析每一张图片的信息
        if self.target_transform is not None:
            ann=self.target_transform(anno)
        ann=np.array(ann).reshape(-1,5) if len(ann)>0 else(np.zeros((0,5),dtype=np.float32))
